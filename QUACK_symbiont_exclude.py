...
for col_no in range(5,COL_NO):
    for row_no in range(1,ROW_NO):
        if TABLE[0][col_no] in Blank_PCR:
            if "Wolbachia" in TABLE[row_no][2]:
                if "Rickettsia" in TABLE[row_no][2]:
                    if "Sodalis" in TABLE[row_no][2]:
                        if "Providencia" in TABLE[row_no][2]:
                            if "endosymbiont" in TABLE[row_no][2]:
                                TABLE[row_no][col_no] = 0
...
for row_no in range(1, len(TABLE[:-1])):
    ...
    for col_no in range(5, COL_NO):
        if TABLE[-1][col_no] == 0:
            ...
            relabund = float(TABLE[row_no][col_no])/TABLE[-1][col_no]
        ###print(row_no, col_no, TABLE[row_no][col_no])###
            if TABLE[0][col_no] in Blank_PCR:
                ...
                if max_r_real < relabund:
                    max_r_real = relabund
    print(max_r_PCR, max_r_extr, max_r_real)  
    ...
    elif max_r_real > (max_r_extr + max_r_PCR) * int(ThresholdA):
        Decision = "Symbiont"
